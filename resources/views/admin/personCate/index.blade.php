        ...
        <div class="layui-col-md12">
            <div class="layui-card">
                <div class="layui-card-header">
                    ...
                    <button class="layui-btn" onclick="xadmin.open('添加分类','{{ route('personCate_add_show') }}',700,500)"><i class="layui-icon"></i>添加</button>
                </div>
...
<script>
    ...
    layui.use(['laydate','form','table'], function(){
        ...
        table.render({
            elem: '#table'
            ,url: "{{ route('personCate_lists') }}"
            ,cols: [[
                {type: 'checkbox',field: 'left'}
                ,{field:'id', width:80, title: 'ID', sort: true,align: "center"}
                ,{field:'sort', width:80, title: '排序', sort: true,align:"center"}
                ,{field:'cate_name', width:150, title: '名称',align: "center"}
                ,{field:'bg_url',align: "center", title: '背景图',templet: function(d){
                       return  "<span id='pic_" + d.id + "'><img src='" + d.bg_url + "'  style='width: 40px' lay-event='showPic' ></span>";
                    } }
        ...
        table.on('tool(tableTool)', function(obj){
            var data = obj.data;
            if(obj.event === 'edit'){
                xadmin.open('编辑',"/hp/personCate/editShow/"+data.id,600,650);
    ...
    function member_stop(id,status){
        ...
        layer.confirm(msg,function(index){
            $.ajax({
                ...
                type: 'post',
                url: "{{ route('personCate_update_status') }}",
                dataType: 'json',
